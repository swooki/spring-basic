...
public class JdbcTemplateProductDao implements ProductDao {
	...
	static RowMapper<Product> productRowMapper = (rs, rowNum) -> {
	...
	public void addProduct(Product p) throws DaoException {
		...
		template.update(sql, p.getProductId(), p.getProductName(), p.getSupplierId(), p.getCategoryId(),
				p.getQuantityPerUnit(), p.getUnitPrice(), p.getUnitsInStock(), p.getUnitsOnOrder(), p.getReorderLevel(),
				p.getDiscontinued());
	}
	...
	public void updateProduct(Product p) throws DaoException {
		String sql = "update products set product_name=?, supplier_id=?, category_id=?, quantity_per_unit=?, "
				...
				+ "where product_id = ? ";
		template.update(sql, p.getProductName(), p.getSupplierId(), p.getCategoryId(), p.getQuantityPerUnit(),
				p.getUnitPrice(), p.getUnitsInStock(), p.getUnitsOnOrder(), p.getReorderLevel(), p.getDiscontinued(),
				p.getProductId());
	}
